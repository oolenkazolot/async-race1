...
class WinnersPage {
  ...
  public async draw(page?: number): Promise<void> {
    if (page) {
      this.page = page;
    }
    ...
    mainEl.textContent = "";
    const winnersEl: HTMLElement = document.createElement("section");
    winnersEl.classList.add("winners");
    const winners: IWinnersData | undefined = await this.controller.getWinners(
      this.page,
      this.limitWinners,
      this.sort,
      this.direction
    );
    const titleEl: HTMLElement = this.createTitle(winners?.countWinners);
    const paginationEl: HTMLElement | undefined = this.createPagination(
      winners
    );
    const winnersTable: HTMLTableElement = await this.createWinnersTable(
      winners
    );
    if (paginationEl) {
      winnersEl.append(titleEl, paginationEl, winnersTable);
    }

    mainEl.append(winnersEl);
  }
  ...
  private createTitle(count: number | undefined): HTMLElement {
    const titleEl: HTMLElement = document.createElement("h2");
    titleEl.classList.add("winners__title");
    titleEl.textContent = `Winners (${count || 0})`;
    return titleEl;
  }

  private createPagination(
    winners: IWinnersData | undefined
  ): HTMLElement | undefined {
    if (winners) {
      const paginationEl: HTMLElement = this.pagination.createPagination(
        this.getPage.bind(this),
        this.getLimitWinners.bind(this),
  ...
  private async createWinnersTable(
    winners: IWinnersData | undefined
  ): Promise<HTMLTableElement> {
    const tableEl: HTMLTableElement = document.createElement("table");
    tableEl.classList.add("winners__table");
    const columnsBlock: HTMLElement = this.createColumnsBlock();
    tableEl.append(columnsBlock);
    const carsAll: IAutos[] | undefined = await this.controller.getAutosAll();
    if (winners && carsAll) {
      winners.winners.forEach((winner: IWinner, index: number) => {
        const car: IAutos | undefined = carsAll.find(
          (car) => car.id === winner.id
        );
        if (car) {
          const rowsBlock: HTMLElement = this.createRowsBlock(
            (this.page - 1) * this.limitWinners + (index + 1),
            car.name,
            winner.wins,
            winner.time,
            car.color
          );
          tableEl.append(rowsBlock);
        }
      });
    }
    return tableEl;
  ...
  private createColumnsBlock(): HTMLElement {
    const namesColumns: { id: string; title: string }[] = [
      { id: "number", title: "Number" },
      { id: "car", title: "Car" },
      { id: "name", title: "Name" },
      { id: "wins", title: "Wins" },
      { id: "time", title: "Best time (seconds)" },
    ];
    const columnsBlock: HTMLElement = document.createElement("tr");
    columnsBlock.classList.add("winners__columns");
    const columns: HTMLElement[] = namesColumns.map((columnName) => {
      const column: HTMLElement = this.createColumn(columnName);
      return column;
    });

    columnsBlock.append(...columns);
    return columnsBlock;
  }

  private createColumn(content: { id: string; title: string }): HTMLElement {
    const sorting = ["wins", "time"];
    const columnEl: HTMLElement = document.createElement("th");
    if (sorting.includes(content.id)) {
      columnEl.addEventListener("click", () => {
        this.setSort(content.id);
      });
      columnEl.classList.add("is-sort");
      if (content.id === this.sort) {
        const className = this.direction === "ASC" ? "sort-asc" : "sort-desc";
        columnEl.classList.add(className);
      }
    }
    columnEl.textContent = content.title;
    return columnEl;
  }

  private createRowsBlock(
    number: number,
    name: string,
    wins: number,
    time: number,
  ...
  public getLimitWinners(): number {
    return this.limitWinners;
  }
}
